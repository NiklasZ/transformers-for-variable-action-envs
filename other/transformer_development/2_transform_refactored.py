...
if __name__ == "__main__":
    ...
    using_wandb = args['command'] == 'resume' or args['prod_mode']
    ...
    args['batch_size'] = int(args['num_envs'] * args['num_envs'])
    ...
    envs, ai_opponent_names = create_envs(args)
    ...
    obs = torch.zeros(
        (num_steps, num_envs, observation_shape[0] * observation_shape[1], observation_size)).to(device)
    obs_masks = torch.ones((num_steps, num_envs, observation_shape[0] * observation_shape[1]),
                           dtype=torch.bool).to(device)
    entity_counts = torch.zeros((num_steps, num_envs), dtype=torch.long).to(device)
    unit_positions = torch.zeros((num_steps, num_envs, observation_shape[0] * observation_shape[1])).to(device)
    unit_masks = torch.ones((num_steps, num_envs, observation_shape[0] * observation_shape[1]),
                            dtype=torch.bool).to(device)

    actions = torch.zeros((num_steps, num_envs) + action_space_shape).to(device)
    logprobs = torch.zeros((num_steps, num_envs)).to(device)
    rewards = torch.zeros((num_steps, num_envs)).to(device)
    dones = torch.zeros((num_steps, num_envs)).to(device)
    values = torch.zeros((num_steps, num_envs)).to(device)
    invalid_action_masks = torch.zeros((num_steps, num_envs) + invalid_action_shape).to(device)
    ...
    # TRY NOT TO MODIFY: prepare the execution of the game.
        ...
        with torch.no_grad():
            ...
            if args['gae']:
                ...
                for t in reversed(range(num_steps)):
                    ...
                    advantages[t] = lastgaelam = delta + args['gamma'] * args[
        ...
        if args['prod_mode']:
            if not os.path.exists(f"models/{experiment_name}"):
                ...
                wandb.save(f"optimizer.pt")
                wandb.save(f"best_agent.pt")
                wandb.save(f"best_optimizer.pt")
