...
observation_size = 6 # position, HP, resources, owner, unit type and current action
agent = EmbeddedAgent(mapsize, envs, device, args.transformer_layers, args.feed_forward_neurons,
...
next_obs, next_entity_mask, next_entity_count, next_unit_position, next_unit_mask, next_enemy_unit_mask, next_neutral_unit_mask = \
    reshape_observation_embedded(torch.Tensor(envs.reset()).to(device), device)
    ...
    for step in range(0, args.num_steps):
        ...
        with torch.no_grad():
            try:
                ...
                next_obs, next_entity_mask, next_entity_count, next_unit_position, next_unit_mask, next_enemy_unit_mask, next_neutral_unit_mask = \
                    reshape_observation_embedded(torch.Tensor(raw_obs).to(device), device)
